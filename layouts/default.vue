    ...
    <v-app>
        <v-app-bar elevation="0" :border="true" color="appbar">
            ...
            <template v-if="$vuetify.display.mdAndUp">
                ...
                <v-btn class="text-transform-none" href="#portfolio" variant="text" size="large">{{
                        t('portfolio')
                    }}
                </v-btn>
                <translate-switcher></translate-switcher>
                <v-btn class="text-transform-none" variant="text" size="large" @click="toggleTheme"
                       :icon="getTheme"></v-btn>
            </template>

            <template v-else>
                <v-app-bar-nav-icon variant="text"
                                    @click.stop="navigationDrawer = !navigationDrawer"></v-app-bar-nav-icon>
            </template>
